...
#include <time.h>                // 时间处理库（处理时间相关的功能，如计算时间差、获取当前时间等）
#include <windows.h>             // Windows API（提供Sleep函数用于实现等待功能）
static time_t last_request_time = 0;    // 上次请求时间（用于实现请求间隔，避免触发Codeforces API的速率限制）

void wait_for_rate_limit() {            // 等待函数（确保每次请求之间至少间隔2秒，以避免触发Codeforces API的速率限制）
    time_t now = time(NULL);                        // 获取当前时间（获取当前的系统时间，单位为秒）
    double diff = difftime(now, last_request_time); // 计算当前时间与上次请求时间的差值（单位为秒）
    
    // 确保至少间隔 2 秒
    if (diff < 2.0) {
        int sleep_ms = (int)((2.0 - diff) * 1000);
        Sleep(sleep_ms);                            // Windows平台的睡眠函数，参数为毫秒（如果当前时间与上次请求时间的差值小于2秒，则计算需要等待的时间并调用Sleep函数暂停程序执行）
    }
...
int fetch_user_info(const char* handle, int* rating, char* title, char* avatar_url) {
    ...
    if (res == CURLE_OK) {                         // 如果HTTP请求成功，解析JSON响应并提取用户信息
        ...
        if (json) {
            ...
            if (status && status->valuestring && strcmp(status->valuestring, "OK") != 0) {
                // API返回错误状态（如用户不存在）
                cJSON_Delete(json);
                free(mem.data);
                curl_easy_cleanup(curl);
            ...
            if (result && cJSON_IsArray(result) && cJSON_GetArraySize(result) > 0) {                     // 如果result字段存在且是数组，提取第一个用户的信息（Codeforces API允许一次请求获取多个用户的信息，但这里我们只请求了一个用户，所以取第一个元素）
            ...
            if (contests_json && cJSON_IsArray(contests_json)) {
                ...
                *count = total < MAX_CONTEST ? total : MAX_CONTEST;  // 限制比赛数量（如果比赛数量超过MAX_CONTEST，则只处理前MAX_CONTEST条记录，防止内存溢出）
                ...
                for (int i = 0; i < *count; i++) {                       // 遍历比赛记录（循环处理每一条比赛记录，提取比赛信息并存储在contests数组中）
                    ...
                    cJSON* contestId = cJSON_GetObjectItem(c, "contestId");
                    ...
                    contests[i].id = contestId ? contestId->valueint : 0;
                    ...
                    strcpy(contests[i].contestProblems, "");
                    strcpy(contests[i].practiceProblems, "");
                }
            }
            cJSON_Delete(json);
        }
    ...
    *count = 0;
    ...
    while (from <= MAX_SUBMISSIONS) {
        ...
        for (int i = 0; i < n && *count < MAX_SUBMISSIONS; i++) {
            ...
            cJSON* contestId = cJSON_GetObjectItem(s, "contestId");
            if (!v || !a || !t) continue;

            strcpy(subs[*count].verdict, v->valuestring ? v->valuestring : "");
            subs[*count].creationTimeSeconds = t->valueint;
            subs[*count].contestId = contestId ? contestId->valueint : 0;
            ...
            cJSON* index_obj = NULL;
            if (problem) {
                rating_obj = cJSON_GetObjectItem(problem, "rating");
                index_obj = cJSON_GetObjectItem(problem, "index");
            }
            subs[*count].problemRating = rating_obj ? rating_obj->valueint : -1;
            strcpy(subs[*count].problemIndex, index_obj && index_obj->valuestring ? index_obj->valuestring : "");
...
// 判断问题索引是否已存在于字符串中
int problem_exists(const char* problems, const char* idx) {
    char search[16];
    snprintf(search, sizeof(search), "%s", idx);
    return strstr(problems, search) != NULL;
}
...
void analyze_submissions(struct Submission* subs, int sub_cnt, struct Contest* contests, int contest_cnt,
    ...
    *practice_count = 0;
    ...
    for (int i = 0; i < sub_cnt; i++) {
        ...
        // 查找对应的比赛
        for (int j = 0; j < contest_cnt; j++) {
            if (subs[i].contestId == contests[j].id) {  // 通过比赛ID匹配
                if (strcmp(subs[i].verdict, "OK") == 0 && strlen(subs[i].problemIndex) > 0) {
                    long long contestTime = contests[j].time;
                    long long subTime = subs[i].creationTimeSeconds;
                    
                    // 假设比赛通常在rating更新时间前进行，比赛持续约3小时
                    // 检查提交是否在比赛期间（假设比赛在rating更新时间前约4小时开始）
                    long long estimatedContestEnd = contestTime;  // rating更新时间作为比赛结束估计
                    long long estimatedContestStart = estimatedContestEnd - 4 * 3600;
                    
                    if (subTime >= estimatedContestStart && subTime <= estimatedContestEnd + 3600) {
                        // 比赛期间通过
                        if (!problem_exists(contests[j].contestProblems, subs[i].problemIndex)) {
                            if (strlen(contests[j].contestProblems) < 58) {
                                if (strlen(contests[j].contestProblems) > 0) strcat(contests[j].contestProblems, ", ");
                                strncat(contests[j].contestProblems, subs[i].problemIndex, 58 - strlen(contests[j].contestProblems));
                            }
                            contests[j].contestAC++;
                        }
                    } else {
                        // 赛后补题
                        if (!problem_exists(contests[j].practiceProblems, subs[i].problemIndex) && 
                            !problem_exists(contests[j].contestProblems, subs[i].problemIndex)) {
                            if (strlen(contests[j].practiceProblems) < 58) {
                                if (strlen(contests[j].practiceProblems) > 0) strcat(contests[j].practiceProblems, ", ");
                                strncat(contests[j].practiceProblems, subs[i].problemIndex, 58 - strlen(contests[j].practiceProblems));
                            }
